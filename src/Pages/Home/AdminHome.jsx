      ...
      queryClient.setQueryData(["users"], (oldUsers) =>
        oldUsers.map((user) =>
          user._id === userId
            ? {
                ...user,
                status: action === "activate" ? "approved" : action === "block" ? "blocked" : user.status,
              }
            : user
        )
      );
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-primary">
      ...
      <div className="text-center mb-4 pt-4">
        <h1 className="text-4xl font-extrabold text-white p-4">User Management</h1>
        <input
          type="text"
          ...
          <tbody className="text-white">
            {filteredUsers.map((user) => (
              <tr key={user._id}>
                ...
                <td className="py-2 px-4 border border-gray-300">{user.mobileNumber}</td>
                <td className="py-2 px-4 border border-gray-300 capitalize">{user.role}</td>
